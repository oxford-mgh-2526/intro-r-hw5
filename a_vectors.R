...
ii <- as.integer(1) # << edit #In R: 1 is double not integer
stopifnot(is.integer(ii) && ii == 1) #check whether ii is a integr and ii also =1

vec1 <- 1:3
vec2 <- c(vec1, vec1, 2L * vec1)# << edit #In R: 2 is not integer
...
ss <- 15 # << edit
stopifnot(sum(seq_along(vec6))==ss)
