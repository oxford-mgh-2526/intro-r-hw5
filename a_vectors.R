...
ii <- as.integer(1) # << done
stopifnot(is.integer(ii) && ii == 1)

vec1 <- 1:3
vec2 <- as.integer(c(vec1, vec1, 2 * vec1)) # << done
...
vec4 <- letters[4:1]
str <- paste(vec4[4], vec4[3], sep = "-") # << done
stopifnot(str == "a-b")

str <- paste(vec4[3], vec4[3], sep = "") # << done
